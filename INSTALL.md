...
Query the `/rs/settings/appSettings?filters=hidden==true%26%26key=={SETTING_KEY}&offset=0&max=1000` endpoint to find the default value and UUID for each setting (replace `{SETTING_KEY}` placeholder with the name of the setting). 
